...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const renderSidebarNavigation = () => {
    ...
    console.log('🔍 [DEBUG] Role config pages:', roleConfig.pages);
    const navHtml = roleConfig.pages.map(page => {
      const pageConfig = roleConfig.pageConfigs?.[page] || {};
      const label = pageConfig.label || page.replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase());
      const icon = pageConfig.icon || 'circle';
      return `<a href="${buildHash(appState.currentRole, page)}" class="nav-link flex items-center gap-3 px-4 py-3 text-sm font-medium text-gray-600 hover:text-gray-900 hover:bg-gray-50 rounded-lg transition-colors" data-page="${page}">
    ...
    const profileHtml = `
      <div class="flex items-center gap-3 px-4 py-3">
        <div class="w-8 h-8 bg-indigo-100 rounded-full flex items-center justify-center">
          ${getIcon('user', 'w-4 h-4 text-indigo-600')}
        </div>
        <div class="flex-1 min-w-0">
          <p class="text-sm font-medium text-gray-900 truncate">${appState.loginEmail || 'User'}</p>
