...
- **Login redirect (11 Nov 2025):** Кнопка `Sign in` маршрутизирует пользователя в App Router по выбранной роли (`operations → /operations/fleet-calendar`, `sales → /sales/fleet-calendar`, `ceo → /exec/dashboard`, `driver → /driver/tasks`) с валидацией email, пока Supabase Auth не подключен.
