-- Update run_kommo_full_refresh to map responsible user (06 Dec 2025)
...
begin
  ...
  with lead_stage as (
    select
      ...
      (l.payload->>'responsible_user_id')::text as responsible_user_id,
      l.kommo_created_at,
      l.kommo_updated_at
    ...
    left join public.staff_accounts s on s.external_crm_id = ls.responsible_user_id
  )
  insert into public.sales_leads(lead_code, client_id, owner_id, value_amount, created_at, updated_at)
  ...
  insert into public.bookings (external_code, client_id, owner_id, total_amount, deposit_amount, source_payload_id, created_at, updated_at, channel)
  select 'K-' || lead_id,
         client_id,
         owner_id,
         price,
  ...
  from (
    ...
    left join public.staff_accounts s on s.external_crm_id = (l.payload->>'responsible_user_id')::text
    where l.run_id = p_run_id
  ) as bookings_data
