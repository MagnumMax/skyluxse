...
declare
  ...
  v_failed boolean := (new.status = 'failed');
begin
  ...
  values (v_bucket, v_type, 1, case when v_failed then 0 else 1 end, case when v_failed then 1 else 0 end, new.created_at)
  on conflict (bucket_start, event_type) do update
    set total_events = public.kommo_webhook_stats.total_events + 1,
        processed_events = public.kommo_webhook_stats.processed_events + (case when v_failed then 0 else 1 end),
        failed_events = public.kommo_webhook_stats.failed_events + (case when v_failed then 1 else 0 end),
