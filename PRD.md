...
### 11. Рабочее место Sale Manager
**FR-038: 360°-карточка клиента**
- Полные контактные данные с уровнем лояльности (New, Returning, VIP) и каналом привлечения
- История аренд, активные и будущие бронирования с быстрым переходом к деталям поездки
- Предпочтения клиента по автомобилям, локациям доставки/возврата, дополнительным услугам и страховкам
...
**FR-039: Единое окно оформления бронирования**
- Подбор автомобиля по фильтрам (класс, бюджет, популярность) и живой доступности на выбранные даты
- Автоматический расчет тарифов за сутки/час, сезонных коэффициентов, депозитов и промокодов с пересчетом итоговой суммы (FR-028)
- Предложение дополнительных услуг: доставка до двери/аэропорта, полное КАСКО, дополнительный водитель, детские кресла
- Быстрое создание задач подготовки и доставки для операций с учетом выбранных слотов (FR-013)

**FR-040: Интерактивный таймлайн сделки**
- Лента этапов: консультация, проверка документов, оплата, подготовка авто, доставка/самовывоз, возврат, удержание/возврат депозита
...
- История коммуникаций с клиентом (WhatsApp, звонки, email) и заметки менеджера
...
- Добавление пакета в бронирование в один клик с пересчетом цены и синхронизацией с Zoho (FR-017)
...
- AI рекомендации по upsell (ближайшие апгрейды, страховки, услуги водителя) на основе профиля клиента и текущего заказа

**FR-043: Аналитические срезы для продаж**
- Дашборды по причинам отказов (цена, депозит, недоступность авто, документы) и каналу привлечения
- Популярность автомобилей по сегментам клиентов (туристы, резиденты, повторные) и по сезонам
- Сравнение эффективности промокодов, партнерских и рекламных кампаний
- SLA метрики: время ответа, скорость подтверждения, конверсия из запроса в бронирование с целевыми показателями
