...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: booking ? `${booking.code} · Booking detail` : `Booking ${params.bookingId}`,
  }
}

export default async function OperationsBookingDetailPage({ params }: PageProps) {
  const booking = await getLiveBookingById(params.bookingId)
  if (!booking) {
    notFound()
  }
  ...
  return <OperationsBookingDetail booking={booking} client={client ?? undefined} driver={driver ?? undefined} />
}
