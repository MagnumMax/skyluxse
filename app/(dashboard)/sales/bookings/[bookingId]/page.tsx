...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: booking ? `${booking.code} · Sales booking` : `Sales booking ${params.bookingId}`,
  }
}

export default async function SalesBookingDetailPage({ params }: PageProps) {
  const booking = await getLiveBookingById(params.bookingId)
  if (!booking) {
    notFound()
  }
  ...
  return <OperationsBookingDetail booking={booking} client={client ?? undefined} driver={driver ?? undefined} variant="sales" />
}
