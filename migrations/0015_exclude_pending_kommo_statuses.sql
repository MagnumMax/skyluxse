...
declare
  ...
  v_excluded_statuses constant bigint[] := array[79790631::bigint, 91703923::bigint];
begin
  ...
  with valid_booking_leads as (
    select *
    from public.stg_kommo_leads
    where run_id = p_run_id
      and coalesce((payload->>'status_id')::bigint, 0) <> all (v_excluded_statuses)
  ), vehicle_links as (
